...
if($remotehost) {
    ...
    //Check API key & ip
    if($ip !=$opt['allowed_ip'] || $key !=$opt['api_key'] || $opt['allowed_ip'] =='' || $opt['api_key']=='' || !isset($opt['allow_piping'])) { 
        api_exit(EX_NOPERM,'Unknown remote host ['.$ip.'] or invalid API key ['.$key.'] or piping not set up');
    	}
